...
$(document).on("click", ".token-box", function () {
    ...
    $('html, body').animate({scrollTop: scrollto}, 0, 'easeInOutExpo');
...
$(window).on('load', function () {
    var gcTokens = ['gcDAI', 'gcUSDC', 'gcBAT', 'gcETH', 'gcREP', 'gcSAI', 'gcUNI',  'gcUSDT', 'gcWBTC', 'gcZRX'];
    var tokens = ['dai', 'usdc'];
    var gaTokens = ['gaBAT', 'gaBUSD', 'gaDAI', 'gaENJ', 'gaKNC', 'gaLEND', 'gaLINK', 'gaMANA', 'gaMKR', 'gaREN',
        'gaREP', 'gaSNX', 'gaSUSD', 'gaTUSDT', 'gaUNI', 'gaUSDC', 'gaUSDT', 'gaWBTC', 'gaYFI', 'gaZRX'];
    ...
    gcTokens.forEach(each => {
        gcTokensHtml = gcTokensHtml + `<div class="col-lg-2 col-md-12 tokens-info-col">
                    <div class="count-box token-box" data-token="${each}">
                        <img src="security/img/gcTokens/${each}.png" class="token-info-img">
                        <span>NA</span>
                        <span>${each}</span>
                        <p>Info</p>
                    </div>
    ...
    tokens.forEach(each => {
        tokensHtml = tokensHtml + `<div class="col-lg-2 col-md-12 tokens-info-col">
                    <div class="count-box token-box" data-token="${each}">
                        <img src="security/img/tokens/${each}.png" class="token-info-img">
                        <span>NA</span>
                        <span>${each}</span>
                        <p>Info</p>
                    </div>
    ...
    gaTokens.forEach(each => {
        gaTokensHtml = gaTokensHtml + `<div class="col-lg-2 col-md-12 tokens-info-col">
                    <div class="count-box token-box" data-token="${each}">
                        <img src="security/img/gaTokens/${each}.png" class="token-info-img">
                        <span>NA</span>
                        <span>${each}</span>
                        <p>Info</p>
                    </div>
    ...
    gtokens.foreach(each => {
        gtokenshtml = gtokenshtml + `<div class="col-lg-2 col-md-12 tokens-info-col">
                    <div class="count-box token-box" data-token="${each}">
                        <img src="security/img/gtokens/${each}.png" class="token-info-img">
                        <span>NA</span>
                        <span>${each}</span>
                        <p>info</p>
                    </div>
